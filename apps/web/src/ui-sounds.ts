...
export function startGameMusic(): () => void {
  ...
  const scheduleCycle = (cycleStart: number): void => {
    ...
    [0, 16].forEach((melodyStart, melodyIndex) => {
      const melody = JAZZ_MELODIES[melodyIndex] ?? [];
      melody.forEach(([offset, frequency]) => {
        schedulePianoNote(audio, master, frequency, cycleStart + melodyStart + offset, 0.92, 0.012, "sine");
        schedulePianoNote(audio, master, frequency * 2, cycleStart + melodyStart + offset, 0.68, 0.003, "triangle");
      });
    });
